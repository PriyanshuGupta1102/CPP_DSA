...
bool dfs(TreeNode* root){
    ...
    bool rootIsBal = abs(height(root->left) - height(root->right)) <= 1 ? true : false;

    return leftIsBal && rightIsBal && rootIsBal;
}

int main() {
    TreeNode* root = buildTree();

    dfs(root) ? cout << "height-balanced!" << endl : cout << "not height balaned!" << endl;
